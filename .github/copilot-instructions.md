...
When you create an implementation plan, save it as a Markdown file under `docs/plans` so the user can review it after the session.
...
- When referring to a previous plan, search `docs/plans/` first and update the relevant file when continuing that work.

Use memory for brief cross-session preferences or facts only. The durable record of implementation plans belongs in `docs/plans/`.
